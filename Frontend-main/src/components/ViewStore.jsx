...
export default function ViewStore() {
    ...
    const handleDelete = () => {
        if (selectedStore && window.confirm("Are you sure you want to delete this store?")) {
    ...
    return (
        <div className="w-full min-h-screen bg-gray-50 flex flex-col lg:flex-row relative overflow-hidden">
            {/* MAIN CONTENT AREA */}
            <div className={`flex-1 p-4 md:p-8 transition-all duration-300 ${isDetailsOpen ? 'lg:mr-[400px]' : ''} overflow-y-auto h-screen`}>

                {/* Header Section */}
                <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-8">
                    <div>
                        <h1 className="text-3xl md:text-4xl font-extrabold text-gray-900 tracking-tight">Stores</h1>
                        <p className="text-gray-500 mt-1">Manage your multi-vendor stores.</p>
                    </div>
                            ...
                            >
                                ...
                                <div className="flex-1 flex flex-col">
                                    <div className="flex justify-between items-start mb-1">
                                        <h3 className="font-bold text-gray-900 line-clamp-1 text-lg group-hover:text-blue-600 transition-colors">{store.name}</h3>
                                    </div>
                                    ...
                                    <div className="mt-auto flex items-center justify-between">
                                        <span className="text-sm font-semibold text-gray-500">{store.currency}</span>
                                        <div className={`text-xs font-medium px-2 py-1 rounded-md ${store.isPublished ? 'bg-green-100 text-green-700' : 'bg-yellow-100 text-yellow-700'}`}>
          ...
          ${isDetailsOpen || selectedStore ? 'translate-x-0' : 'translate-x-full'}
                        ...
                        <div className="flex-1 overflow-y-auto p-6 space-y-6">
                            ...
                            <div className="space-y-5">
                                ...
                                <div className="grid grid-cols-2 gap-4">
                                    ...
                                    <div>
                                        <label className="block text-xs font-bold text-gray-500 uppercase tracking-wider mb-1">Commission</label>
                                        <input
                                            className="w-full bg-gray-50 border-transparent focus:bg-white focus:border-black focus:ring-0 rounded-lg p-3 text-gray-900 transition-all"
                                ...
                                <div>
                                    <label className="block text-xs font-bold text-gray-500 uppercase tracking-wider mb-1">Owner ID</label>
                                    <input
                                ...
                                <div className="pt-4 border-t border-gray-100">
                                    <div className="flex justify-between items-center text-sm text-gray-500">
                                        <span>Status</span>
                                        <span className={`font-bold ${selectedStore.isPublished ? 'text-green-600' : 'text-yellow-600'}`}>
